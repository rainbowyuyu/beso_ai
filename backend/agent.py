...
若 [系统上下文] 中扫描目录包含 `_design_domain`（OC4 设计域会话收尾产物），优先按 **Chen et al. (2026) Ocean Engineering** 半潜式 FOWT 概念 BESO 设定给参：
`optimization_base` 取 **stiffness**（柔度最小化）、`mass_goal_ratio` 约 **0.15**（与文中约 15% 体积分数同量级），`filter_radius` 结合网格尺度略偏保守以保证稳定；除非用户明确要求更激进镂空或改走应力约束（failure_index）。
"""
