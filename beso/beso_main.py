...
exec(open(os.path.join(beso_dir, "beso_conf.py"), encoding="utf-8").read())

symmetry_120_pairs = []
try:
    _sym_map_path = symmetry_120_map_path
except NameError:
    _sym_map_path = ""
if _sym_map_path:
    import json as _json_sym

    _sym_map_path = os.path.normpath(os.path.expandvars(str(_sym_map_path)))
    if not os.path.isabs(_sym_map_path):
        _sym_map_path = os.path.normpath(os.path.join(beso_dir, _sym_map_path))
    if os.path.isfile(_sym_map_path):
        with open(_sym_map_path, encoding="utf-8") as _sf:
            _raw = _json_sym.load(_sf)
        if isinstance(_raw, dict) and "pairs" in _raw:
            symmetry_120_pairs = [[int(a), int(b)] for a, b in _raw["pairs"]]
        elif isinstance(_raw, list):
            symmetry_120_pairs = [[int(a), int(b)] for a, b in _raw]
        print("Loaded symmetry_120_map: {} slave<-master pairs from {}".format(len(symmetry_120_pairs), _sym_map_path))
    else:
        print("WARNING: symmetry_120_map_path not found: {}".format(_sym_map_path))


def _apply_symmetry_120_to_elm_states(es):
    if not symmetry_120_pairs:
        return
    for pair in symmetry_120_pairs:
        slave, master = int(pair[0]), int(pair[1])
        if master in es:
            es[slave] = es[master]
...
while True:
    ...
    _apply_symmetry_120_to_elm_states(elm_states)
