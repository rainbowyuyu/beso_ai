...
path_calculix = "C://soft//PrePoMax0.4.7//Solver//ccx_2.13_MT"  # path to the CalculiX solver
...
file_name = "Analysis-1.inp"  # file with prepared linear static analysis

elset_name = "design_space"  # string with name of the element set in .inp file (CASE sensitive!)
domain_optimized[elset_name] = True  # True - optimized domain, False - elements will not be removed
domain_density[elset_name] = [7.833e-12, 7.833e-6] #g/mm3 # equivalent density of the domain material for states of switch_elm
...
domain_FI[elset_name] = [[("stress_von_Mises", 1000.0e6)],  # inner tuples () for separate Failure indices
                         [("stress_von_Mises", 1000.0)]]  # new inner list [] for the next state of switch_elm
...
domain_material[elset_name] = ["*ELASTIC \n200000e-6,  0.27",  # material definition after CalculiX *MATERIAL card, use \n for line break
                               "*ELASTIC \n200000,  0.27"]  # next string for the next state of switch_elm
...
elset_name = "nondesign_space"  # string with name of the element set in .inp file (CASE sensitive!)
...
filter_list = [["simple", 10]]  # [[filter type, range, domains or nothing for all domains], [next filter type, range, "domain1", "domain2"], ...]
                            # filter types:
...
mass_addition_ratio = 0.04  # mass to be added in each iteration
mass_removal_ratio = 0.08  # mass to be removed in each iteration
