...
def import_inp(file_name, domains_from_config, domain_optimized, shells_as_composite):
    ...
    try:
        f = open(file_name, "r", encoding="utf-8", errors="replace")
    except IOError:
    ...
    while line != "":
        ...
        if line.strip() == '':
            ...
            if line[:8].upper() == "*INCLUDE":
                ...
                f_include = open(include, "r", encoding="utf-8", errors="replace")
                continue
            read_node = False
            elm_category = []
            elm_2nd_line = False
...
def write_inp(file_name, file_nameW, elm_states, number_of_states, domains, domains_from_config, domain_optimized,
    ...
    if reference_points == "nodes":
        fR = open(file_name[:-4] + "_separated.inp", "r", encoding="utf-8", errors="replace")
    else:
        fR = open(file_name, "r", encoding="utf-8", errors="replace")
    try:
        lines = fR.readlines()
    finally:
        fR.close()
